...
type VendorCatalogCreateFormProps = {
    ...
    catalogID: number,
    defaultValue: CatalogItem | undefined,
}

export default function VendorCatalogUpdateForm({ children, dialogToggle, subfields, catalogID, defaultValue }: VendorCatalogCreateFormProps) {

    const [ subfieldSelects, setSubfieldSelects ] = useState<string[]>([])
    const [ subfieldsReady, setSubfieldsReady ] = useState(false)
    ...
    const form = useForm<z.infer<typeof CatalogUpdateSchema>>({
    ...
    const sendFormData = (data: z.infer<typeof CatalogUpdateSchema>) => {
        ...
        axios.patch(route('api.catalog.update'), input)
    ...
    useEffect(() => {
        if (typeof defaultValue !== 'undefined' && typeof defaultValue.subfields_collection !== 'undefined' && defaultValue.subfields_collection.length > 0 && subfields.length > 0) {
            const subfieldsArray: string[] = []
            defaultValue.subfields_collection.forEach((subfield) => {
                subfieldsArray.push(subfield.subfield_id.toString())
            })
            setSubfieldSelects(subfieldsArray)
            setSubfieldsReady(true)
        }

        if (typeof defaultValue !== 'undefined' && !defaultReady) {
            const parsed = CatalogUpdateSchema.safeParse({
                ...
                registerDateStart: new Date(defaultValue.register_date_start),
                registerDateEnd: new Date(defaultValue.register_date_end),
            })
            if (!parsed.success) {
    ...
    }, [defaultValue, subfields, defaultReady, form])
...

                    {
                        (subfieldsReady) ? (
                            ...
                            <div className="mb-8">
                                ...
                                <MultiSelect id="createSubfields" defaultValue={subfieldSelects} options={subfields} onValueChange={multiSelectChange} placeholder="Pilih Sub Bidang" variant="default" maxCount={0} modalPopover={true}></MultiSelect>
                            </div>
